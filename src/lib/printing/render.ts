    ...
    <div class="permit">
      ...
      <div class="watermark" aria-hidden="true">
        <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
          ...
          <circle cx="100" cy="100" r="70" fill="none" stroke="#0f766e" stroke-width="1"/>
          <circle cx="100" cy="100" r="50" fill="none" stroke="#0f766e" stroke-width="1"/>
        </svg>
      </div>
      ...
      <section class="identity-strip">
        <div class="field">
          ...
          <strong class="value mono">${esc(doc.permitNumber)}</strong>
        </div>
        <div class="field">
          <span class="label">Registration Plate</span>
          <strong class="value mono">${esc(doc.registrationNumber)}</strong>
        </div>
        <div class="field">
          <span class="label">Fleet VIC</span>
          <strong class="value mono">${esc(doc.vic)}</strong>
        </div>
      </section>
      ...
      <section class="permit-body">
        <div class="left-column">
          ...
          <div class="section">
            ...
            <div class="pair">
              <strong>Route:</strong> ${esc(doc.routeOrigin)} → ${esc(doc.routeDestination)} (${esc(doc.routeRegion)} Region)
            </div>
          </div>
        ...
        <aside class="right-column">
          ...
          <div class="validity-block">
            <div class="validity-row">
              ...
              <strong class="mono">${esc(doc.permitIssueDate)}</strong>
            </div>
            <div class="validity-row">
              <span class="label">Expiry Date</span>
              <strong class="mono highlight">${esc(doc.permitExpiryDate)}</strong>
            </div>
            <div class="validity-row">
              <span class="label">Status</span>
              <strong class="mono">${esc(doc.permitStatus)}</strong>
            </div>
          </div>
        </aside>
      </section>
      ...
      <footer class="permit-footer">
        ...
        <div class="signature">
          ...
          <div class="auth-hash">
            <span class="mono">AUTH-RTB-${esc(doc.permitNumber).replace(/\W/g, "")}</span>
          </div>
        </div>
      </footer>

      <div class="print-meta">
        Printed ${esc(doc.printTimestamp)} • Verify at ${esc(doc.verifyUrl)}
      </div>
    </div>
  `;
}
...
<head>
<meta charset="utf-8">
<title>Permit ${esc(doc.permitNumber)} — ${esc(doc.registrationNumber)}</title>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .toolbar a.secondary { background: #e5e5e5; color: #111; }
</style>
</head>
<body>

<div class="toolbar no-print">
  <a href="${esc(doc.verifyUrl)}" target="_blank" class="secondary">Test Verification</a>
